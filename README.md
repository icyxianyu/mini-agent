...
核心能力：**对话**、**文件操作**、**终端命令**、**内容搜索**、**上下文注入**。10 个工具 + 流式输出 + 日志。架构采用 Agent + Tools 模式，和真正的 Claude Code 是一套设计。
...
│   ├── context.ts            # 上下文注入（启动时检测项目类型）
│   ├── logger.ts             # 日志模块（毫秒时间戳 + 原始数据）
...
│       ├── search-tools.ts   # 内容搜索工具
│       └── index.ts          # 工具注册表（汇总 10 个工具）
├── logs/                     # 运行时生成的日志
├── .env.example              # 配置模板
├── package.json
└── tsconfig.json
...
启动时自动收集项目信息注入 system prompt：

```
collectProjectContext(".")
  ├─ 项目类型检测: 检查 9 种标记文件 (package.json/go.mod/Cargo.toml...)
  ├─ 配置收集: 读配置文件前 50 行
  ├─ 入口文件: 自动检测并读前 30 行
  ├─ 通用文件: README 前 15 行、.gitignore 等
  ├─ 目录结构: 递归树，3 层深度，30 条目限制
  └─ Git 状态: 分支 + 最近 5 次提交

→ 组装为 ~800 token 的 system prompt 尾部
→ LLM 一启动就知道项目结构，无需额外探索
→ 可通过 ENABLE_CONTEXT_INJECTION=false 关闭
```
...
| `search_content` | "找出所有用了 import 的文件" |
| `execute_command` | "跑一下 npm test"、"git status" |

**上下文注入**：启动时自动检测项目类型（Node/Go/Python/Rust/Java…共 9 种），收集配置、入口文件、README、Git 状态，注入 system prompt，LLM 天然"知道项目长什么样"。
